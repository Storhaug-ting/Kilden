...
Loven regulerer gjerde mellom naboeiendommer: hvor gjerdet skal stå (§ 4),
retten til å ha gjerde man selv betaler (§ 6), når en nabo kan kreve at den
andre er med på gjerdeholdet (§ 7), hvordan kostnadene fordeles mellom dem
(§§ 8–10), og hvem som svarer for gjerdeholdet når en annen enn eieren bruker
eiendommen (§§ 11–13). Gjerdehold ved offentlig veg og jernbane faller utenfor
loven, jf. § 1. Den hører sammen med grannelova, som loven selv viser til.
...
| **Utgiver** | Lovdata |
| **LovId** | `1961-05-05` |
| **Original på nett** | <https://lovdata.no/dokument/NL/lov/1961-05-05> |
...
| **Kildefil i datasettet** | `nl-19610505-000.xml` |
| **Hentet** | 05.08.2026 |
| **SHA256 (lokal snapshot)** | `F7D7418F823EBC361C44ED019BFF0F04FE5FE36B82302299B2631D246500C1B3` |

Loven har ingen løpenummer – den er «lov 5. mai 1961 om grannegjerde» – så
`LovId` er på formen ÅÅÅÅ-MM-DD, og datasettets filnavn får `000` som
løpenummer. Samme mønster som [servituttlova](../servituttlova/README.md).
...
`grannegjerdelova.xml` i denne mappa er derfor ikke en fil Lovdata har
publisert et sted vi kan lenke til – det er en **snapshot**: den XML-en Lovdata
sitt API serverte for denne loven da datasettet ble hentet, kopiert inn lokalt
slik at den kan diffes mot en frisk henting. `kilde.psd1` sin `Sha256` er
...
`scripts/Update-Lovtekst.ps1 grannegjerdelova -GodtaNyVersjon -Skriv` for å ta
inn en ny versjon når det skjer.
...
| 2 | [grannegjerdelova.xml](grannegjerdelova.xml) | Lokal snapshot av lovens XML fra Lovdata sitt datasett, slik den sto ved hentingen over. |
| 3 | [grannegjerdelova.md](grannegjerdelova.md) | Maskingenerert markdown av XML-en. Denne leses og diffes. |
...
```powershell
./scripts/Update-Lovtekst.ps1 grannegjerdelova -Skriv
```
...
`README.md` og frontmatter i `grannegjerdelova.md`.
